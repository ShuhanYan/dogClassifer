...
def DenseNet(nb_dense_block=4, growth_rate=48, nb_filter=96, reduction=0.0, dropout_rate=0.0, weight_decay=1e-4,
    ...
    x_c = GlobalAveragePooling2D(name='pool' + str(final_stage))(x)
    x_c = Dense(classes, name='fc6')(x_c)
    x_c = Activation('softmax', name='prob')(x_c)

    model = Model(img_input, x_c, name='densenet')
